...
{
    /// <summary>
    /// Builds a <see cref="ShaderReflectionInfo"/> whose bind groups use the given set (set) indices,
    /// each holding a single placeholder binding so the reflection info constructs successfully.
    /// </summary>
    private static ShaderReflectionInfo CreateReflection(params uint[] groups)
    {
    ...
    {
        ShaderReflectionInfo info = CreateReflection();
    ...
    {
        ShaderReflectionInfo info = CreateReflection(0, 1, 2);
        Assert.DoesNotThrow(() => ShaderReflectionUtility.ValidateBindGroupLayouts(info, 8));

        ShaderReflectionInfo atLimit = CreateReflection(0, 1, 2, 3);
    ...
    {
        ShaderReflectionInfo info = CreateReflection(0, 1, 2, 3, 4);
    ...
    {
        ShaderReflectionInfo info = CreateReflection(0, 2);
        ShaderReflectionException ex = Assert.Throws<ShaderReflectionException>(
    ...
    {
        ShaderReflectionInfo info = CreateReflection(1, 2);
    ...
    {
        ShaderReflectionInfo info = CreateReflection(0, 1, 1);
        Assert.Throws<ShaderReflectionException>(
            () => ShaderReflectionUtility.ValidateBindGroupLayouts(info, 8));
    }
