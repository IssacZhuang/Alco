...
{
    /// <summary>
    /// A flat, 4-wide AABB bounding volume hierarchy for 3D workloads (frustum culling,
    /// ray picking, visibility queries), built with the same Morton-code LBVH (Karras 2012)
    /// pipeline as <see cref="NativeBvh3D"/> but shaped after Embree's BVH4:
    /// <para>
    /// - Nodes store the SoA bounds of their (up to) 4 children in a 128-byte block, so one
    ///   Vector128 slab test covers all children (kernels/bvh/bvh_node_aabb.h layout).
    ...
    /// - Leaves are blocks of up to 4 items packed SoA (Embree M-wide primitive blocks);
    ///   unused slots carry empty bounds that fail every intersection test.
    ...
    ///   direction sign (the TravRay organization of node_intersector1.h); besides being
    ///   branch-free in the vector body, this formulation rejects the inverted bounds of
    ///   unused slots, which a min/max slab test cannot.
    /// </para>
    ...
    public unsafe class BvhAabb3D : IDisposable
    {
        ...
        /// <summary>
        /// SoA bounds of the (up to) 4 children plus tagged child references, padded to
        /// 128 bytes (2 cache lines, same as Embree's BVH4 node).
        /// </summary>
        ...
        public struct Node
        {
            ...
            /// <summary>Child lower bounds, Z lane per slot.</summary>
            public Vector128<float> LowerZ;
            /// <summary>Child upper bounds, Z lane per slot.</summary>
        ...
        public struct Leaf
        {
            ...
            public Vector128<float> UpperY;
            /// <summary>Item lower bounds, Z lane per slot.</summary>
            public Vector128<float> LowerZ;
            /// <summary>Item upper bounds, Z lane per slot.</summary>
            public Vector128<float> UpperZ;
        ...
        /// (30-bit codes, LSD radix sort, highest-differing-bit splits), emitting 4-wide
        ...
        public void Build(ReadOnlySpan<BoundingBox3D> bounds)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                Vector3 c2 = bounds[i].Min + bounds[i].Max;
                sceneMin = Vector3.Min(sceneMin, c2);
                sceneMax = Vector3.Max(sceneMax, c2);
            }

            // 30-bit Morton codes packed as (code << 32) | index
        ...
        /// </summary>
        private int BuildRange(int start, int end, int depth, out BoundingBox3D bounds)
        {
            ...
            BoundingBox3D b0 = EmptyBox, b1 = EmptyBox, b2 = EmptyBox, b3 = EmptyBox;
        ...
        private int EmitLeaf(int start, int end, int depth, out BoundingBox3D bounds)
        {
            Item* items = _items.UnsafePointer;
        ...
        // LSD radix sort over the 30-bit codes
        ...
        public void OverlapAabb(in BoundingBox3D query, List<int> results)
        {
            ...
            Vector128<float> qMinZ = Vector128.Create(query.Min.Z);
            Vector128<float> qMaxX = Vector128.Create(query.Max.X);
            Vector128<float> qMaxY = Vector128.Create(query.Max.Y);
            Vector128<float> qMaxZ = Vector128.Create(query.Max.Z);
