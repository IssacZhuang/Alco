...
namespace NLayer
{
    public class MpegFile : IDisposable
    {
        ...
        /// <summary>
        /// Sample count of source Mpeg.
        /// </summary>
        public long SampleCount { get { return _reader.SampleCount; } }
        ...
        int ReadSamplesImpl(Span<float> buffer, int index, int count)
        {
            ...
            while (frame != null)
            {
                readBufferLength = _decoder.DecodeFrame(frame, _readBuf, 0);
                _readBuf.AsSpan(0, readBufferLength).CopyTo(buffer.Slice(cnt));
                cnt += readBufferLength;
                frame.ClearBuffer();
                frame = _reader.NextFrame();
            }
            ...
            //             _readBuf.AsSpan(_readBufOfs, temp).CopyTo(buffer.Slice(index));
