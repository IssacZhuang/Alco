...
public partial class Package
{
	...
	public PackageEntry AddFile(string filePath, byte[] fileData)
	{
		...
		var entry = new PackageEntry(fileName, directory, extension, Crc32.HashToUInt32(fileData), fileData, 0x7FFF);
		// {
		// 	FileName = fileName,
	...
	public void Write(Stream stream)
	{
		...
		try
		{
			...
			if (fileTreeMD5.Hash == null)
			{
				throw new InvalidOperationException("File tree hash is null");
			}
			...
			if (fullFileMD5.Hash == null)
			{
				throw new InvalidOperationException("Full file hash is null");
			}
