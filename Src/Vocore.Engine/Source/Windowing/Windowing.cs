// Origin from the Silk.NET.Windowing.Extensions.Veldrid:
...
namespace Vocore.Engine
{
    ...
    public static class VeldridWindow
    {
        ...
        private static unsafe SwapchainSource GetSwapchainSource(INativeWindow? view)
        {
            if (view is null)
            {
                throw new ArgumentNullException("Null native window");
            }
        ...
        )
        {
            ...
            (
                ...
                x =>
                {
                    //window.GLContext!.GetProcAddress(x);
                    bool success = window.GLContext!.TryGetProcAddress(x, out nint proc);
                    if (!success)
                    {
                        Log.Warning($"Native Proc Address {x} not found");
                    }
                    return success ? proc : default;
                },
