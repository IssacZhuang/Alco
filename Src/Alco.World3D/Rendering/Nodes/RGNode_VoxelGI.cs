...
public readonly struct VoxelGiDescriptor
{
    ...
    /// <summary>
    /// The blue-noise tile bake shader (SSRBlueNoise.slang),
...
public sealed class RGNode_VoxelGI : AutoDisposable, IRenderGraphNode
{
    ...
    // The voxelize pass template (from the descriptor), compiled per material
    // asset surface through the compiler in GetVoxelizeMaterial.
        ...
        in VoxelGiDescriptor descriptor)
    {
        ...
        _attrStatic = rendering.CreateGraphicsBuffer(staticAttributeBytes, "voxel_attr_static_pool");
        _attrDynamic = rendering.CreateGraphicsBuffer(dynamicAttributeBytes, "voxel_attr_dynamic_pool");
        _attributeMemoryBytes = staticAttributeBytes + dynamicAttributeBytes;

        for (int level = 0; level < LevelCount; level++)
        {
            _pageTableStatic[level] = rendering.CreateGraphicsBuffer(pageTableBytes, $"voxel_page_table_static_{level}");
            _pageTableDynamic[level] = rendering.CreateGraphicsBuffer(pageTableBytes, $"voxel_page_table_dynamic_{level}");
            _dirtyBrickCoordinates[level] = rendering.CreateGraphicsBuffer(dirtyBrickBytes, $"voxel_dirty_bricks_{level}");
            ...
            // is 16 bytes.
            _residentBrickCoordinates[level] = rendering.CreateGraphicsBuffer(dirtyBrickBytes * 2, $"voxel_resident_bricks_{level}");
            // Combined page table: 2 uints per slot (static, dynamic).
            _pageTableCombined[level] = rendering.CreateGraphicsBuffer(pageTableBytes * 2, $"voxel_page_table_combined_{level}");
    ...
    private MeshGeometry CreateGeometry(Mesh mesh, uint vertexStrideBytes, in BoundingBox3D localBounds)
    {
        ...
        {
            Vertices = _rendering.CreateGraphicsBuffer(vertexBytes, $"voxel_vertices_{mesh.Name}"),
            Indices = _rendering.CreateGraphicsBuffer(indexBytes, $"voxel_indices_{mesh.Name}"),
