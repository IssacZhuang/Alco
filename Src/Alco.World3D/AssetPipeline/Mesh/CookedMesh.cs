...
/// by <c>StreamableMesh</c> once a LOD's payload has been read; upload via the
/// <c>Upload</c> methods and finalize with <see cref="MarkReady"/>. Consumed by the renderer
/// like any other <see cref="Mesh"/>.
/// </summary>
public sealed unsafe class CookedMesh : Mesh
{
    ...
    public CookedMesh(GPUDevice device, uint vertexBufferSize, uint indexBufferSize, uint vertexStride, int lodIndex, string name = "cooked_mesh")
