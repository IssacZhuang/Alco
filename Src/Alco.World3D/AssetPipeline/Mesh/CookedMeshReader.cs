...
public sealed unsafe class CookedMeshReader : AutoDisposable
{
    ...
    /// <summary>
    /// Open a reader over a cooked mesh held fully in memory (e.g. from a preloaded asset context).
    ...
    public void ReadEntry(string entryName, Span<byte> destination)
    {
        if (!_reader.TryGetEntry(entryName, out PackageEntry? entry))
        {
            throw new InvalidDataException($"Cooked mesh entry '{entryName}' does not exist.");
        }
    ...
    public void ReadChunk(in MeshChunkMeta chunk, SafeMemoryHandle destination)
    {
        ...
        {
            throw new InvalidDataException($"Cooked mesh chunk '{chunk.Entry}' does not exist.");
        }

        if (size != destination.AsReadOnlySpan().Length)
        {
            throw new InvalidDataException(
                $"Cooked mesh chunk '{chunk.Entry}' size mismatch: expected {destination.AsReadOnlySpan().Length}, stored {size}.");
        }
        ...
        {
            throw new InvalidDataException(
                $"Cooked mesh chunk '{chunk.Entry}' hash mismatch: expected {chunk.Hash}, computed {hash}.");
        }
    }
