...
public partial class RenderingSystem
{
    ...
    public unsafe PrimitiveMesh CreatePrimitiveMesh<TVertex>(ReadOnlySpan<TVertex> vertices, ReadOnlySpan<uint> indices, string name = "mesh") where TVertex : unmanaged
    {
        PrimitiveMesh mesh = new PrimitiveMesh(_device, (uint)vertices.Length * (uint)sizeof(TVertex), (uint)indices.Length, IndexFormat.UInt32, name);
    ...
    /// <typeparam name="TVertex">The type of the vertices.</typeparam>
    /// <returns></returns>
    public unsafe PrimitiveMesh CreatePrimitiveMesh<TVertex>(ReadOnlySpan<TVertex> vertices, ReadOnlySpan<ushort> indices, string name = "mesh") where TVertex : unmanaged
    {
