...
// Sun shadow for a voxel: single comparison tap against the cascade atlas with
// a normal + half-voxel receiver offset. Voxels beyond the last split are lit.
...
float SampleSkyVisibility(float3 worldPosition, float4 originAndSize, uint resolution)
{
...
void MainCS(uint3 dispatchId : SV_DispatchThreadID)
{
    ...
    float3 direct = VoxelSkyColor(normal) * SampleSkyVisibility(worldPosition, originAndSize, resolution);

    // Sun (with CSM shadow). The 1/PI matches the direct pass's diffuse BRDF.
    float3 L = normalize(-sunDirection.xyz);
    float NdotL = max(dot(normal, L), 0.0);
    float shadow = lightingParams.x > 0.5 ? SampleSunShadowVoxel(worldPosition, normal, voxelSize) : 1.0;
