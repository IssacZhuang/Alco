...
namespace UniLua
{
    using LuaLogger = UniLua.Tools.LuaLogger;
    ...
    public class LuaTable
    {
		...
		*/
        ...
        {
            LuaLogger.Log("------------------ [DumpParts] enter -----------------------");
            LuaLogger.Log("<< Array Part >>");
            for (var i = 0; i < ArrayPart.Length; ++i)
            {
                var n = ArrayPart[i];
                LuaLogger.Log(string.Format("i:{0} val:{1}", i, n.V));
            }
            LuaLogger.Log("<< Hash Part >>");
            for (var i = 0; i < HashPart.Length; ++i)
            {
                var n = HashPart[i];
                var next = (n.Next == null) ? -1 : n.Next.Index;
                LuaLogger.Log(string.Format("i:{0} index:{1} key:{2} val:{3} next:{4}", i, n.Index, n.Key.V, n.Val.V, next));
            }
            LuaLogger.Log("++++++++++++++++++ [DumpParts] leave +++++++++++++++++++++++");
        }
